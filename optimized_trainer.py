...
"""
Optimized setup and training functions for process mining models
"""
...
def setup_optimized_model(args, df, task_encoder, resource_encoder, device):
    """
    ...
    """
    print(f"{Fore.CYAN}🧠 Setting up optimized {args.model_type} model...{Style.RESET_ALL}")
    
    # Clear memory before creating model
    MemoryOptimizer.clear_memory()
    MemoryOptimizer.print_memory_stats("Before model creation")
    ...
    # Create optimized model based on model type
    ...
    MemoryOptimizer.print_memory_stats("After model creation")
    
    return model


def setup_optimized_optimizer(model, args, class_weights=None, device=None):
    """
...
def train_and_evaluate_optimized_model(model, graphs, args, criterion, optimizer, device, run_dir):
    """
    ...
    MemoryOptimizer.clear_memory()
    MemoryOptimizer.print_memory_stats("Before training")
    ...
    test_metrics = evaluate_optimized_model(model, test_loader, criterion, device, args)
...
def evaluate_optimized_model(model, data_loader, criterion, device, args):
    """
    ...
    try:
        ...
        # Limit to top 20 classes if many classes
        if cm.shape[0] > 20:
            print(f"{Fore.YELLOW}   Too many classes for visualization, showing top 20{Style.RESET_ALL}")
            important_classes = np.unique(np.concatenate([
                y_true.flatten()[:20],  # Some true classes
                y_pred.flatten()[:20]   # Some predicted classes
            ]))[:20]
            
            # Create mask for selected classes
            mask_true = np.isin(y_true, important_classes)
            mask_pred = np.isin(y_pred, important_classes)
        ...
        print(f"{Fore.GREEN}   Saved confusion matrix to {cm_path}{Style.RESET_ALL}")
        
    except Exception as e:
        print(f"{Fore.YELLOW}   Error creating confusion matrix: {e}{Style.RESET_ALL}")
