...
def build_graph_data(df, batch_size=None, verbose=True):
    """
    Convert preprocessed data into graph format for GNN with memory optimization
    ...
    if batch_size is None:
        ...
        estimated_memory_per_case = avg_events_per_case * len(feature_cols) * 8  # Approx bytes per case
    ...
    for i in range(0, num_cases, batch_size):
        ...
        for cid, cdata in batch_df.groupby("case_id"):
            ...
            # Create edges between sequential activities more efficiently
            n_nodes = len(cdata)
            node_counts.append(n_nodes)
            
            if n_nodes > 1:
                # More efficient edge construction
                src = torch.arange(n_nodes-1)
                tgt = torch.arange(1, n_nodes)
                
                # Create bidirectional edges more efficiently
...
def build_enhanced_graph_data(df, batch_size=None, verbose=True):
    """
