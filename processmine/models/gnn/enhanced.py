...
from .positional import PositionalGATConv, PositionalEncoding
from .diverse import DiverseGATConv
import logging
...
class EnhancedGNN(TorchProcessModel):
    """
    ...
    def _compute_loss(self, batch_data, criterion):
        ...
        if self.predict_time:
            ...
            if isinstance(criterion, ProcessLoss):
                # Use time prediction if available
                time_target = batch_data.time if hasattr(batch_data, 'time') else None
                loss, _ = criterion(outputs['task_pred'], graph_labels, 
                                   outputs['time_pred'], time_target)
            else:
                # Fallback to standard classification loss
                loss = criterion(outputs['task_pred'], graph_labels)
        else:
...
class EnhancedGNNWithCritic(TorchProcessModel):
    """
    ...
    def _compute_loss(self, batch_data, criterion):
        """Compute loss for batch"""
        # Delegate to main model
        return self.main_model._compute_loss(batch_data, criterion)
    
    def _extract_predictions(self, outputs, batch_data):
        """Extract predictions from outputs"""
        # Delegate to main model
        return self.main_model._extract_predictions(outputs, batch_data)
