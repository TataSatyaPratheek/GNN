...
class DiverseGATModel(TorchProcessModel):
    """
    ...
    def _compute_loss(self, batch_data, criterion):
        ...
        # Get model outputs
        logits, diversity_loss = self(batch_data.x, batch_data.edge_index, batch_data.batch)
        
        # Get graph-level labels
        graph_labels = get_graph_labels(batch_data.y, batch_data.batch)
        
        # Compute task loss
        task_loss = criterion(logits, graph_labels)
        
        # Combine with diversity loss
    ...
    def _extract_predictions(self, outputs, batch_data):
        ...
        # Get logits (outputs is a tuple of (logits, diversity_loss))
        logits = outputs[0]
